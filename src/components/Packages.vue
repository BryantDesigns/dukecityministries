...
/**
 * The array of tier objects, each representing a package.
 * @type {{ name: string, id: string, href: string, price: { event: string }, description: string, features: string[], featured: boolean, cta: string }[]}
 */
...
const frequencies = [
  { value: 'event', label: 'One Time', priceSuffix: '/event' },
]
  ...
  <div class="bg-white py-24 sm:py-32">
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
      <div class="mx-auto max-w-4xl text-center">
        <h2 class="text-base font-semibold leading-7 text-indigo-600">Ceremony Packages</h2>
      ...
      <div class="isolate mx-auto mt-10 grid max-w-md grid-cols-1 gap-8 lg:mx-0 lg:max-w-none lg:grid-cols-3">
        <div v-for="tier in tiersArray" :key="tier.id" :class="[tier.featured ? 'bg-gray-900 ring-gray-900' : 'ring-gray-200', 'rounded-3xl p-8 ring-1 xl:p-10']">
          <h3 :id="tier.id" :class="[tier.featured ? 'text-white' : 'text-gray-900', 'text-lg font-semibold leading-8']">{{ tier.name }}</h3>
          <p :class="[tier.featured ? 'text-gray-300' : 'text-gray-600', 'mt-4 text-sm leading-6','overflow-auto ...','h-56']">{{ tier.description }}</p>
